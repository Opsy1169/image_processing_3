...
def main():
    ...
    hor_contour = oriented_contour(binary, orientation='horizontal')
    ver_contour = oriented_contour(binary)
    print(np.linalg.norm(inner_contour - inner_contour))
    cv2.imshow("1", inner_contour)
...
def __dilatation(image, size, shape):
    mask = get_square_mask(size) if shape == 'square' else get_cross_mask(size)
    elements_in_mask = size ** 2 - 1 if shape == 'square' else 2 * (size - 1)
...
def __erosion(image, size, shape):
    mask = get_square_mask(size) if shape == 'square' else get_cross_mask(size)
...
def oriented_contour(image, orientation='vertical'):
    image = image / 255
    mask = np.array([[0, 1, 0], [0, 0, 0], [0, -1, 0]]) if orientation != 'vertical' else \
        np.array([[0, 0, 0], [1, 0, -1], [0, 0, 0]])
    conv_result = signal.convolve2d(image, mask, mode='same', fillvalue=1)
    for i in range(conv_result.shape[0]):
        for j in range(conv_result.shape[1]):
            conv_result[i][j] = 0 if conv_result[i][j] == 0 else 1
    return conv_result * 255
